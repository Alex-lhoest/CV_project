...
def sudoku_lines_eval(dilation_parameter):
	...
	for file_number in range (1,79):
	    
	    img_link = 'Data/images/sudoku_%d.png' %file_number
	    annotation_link = 'Data/annotations/sudoku_%d.txt' %file_number
	    ...
	    edges_for_lines = cv2.dilate(imgReduc, np.ones((2,2), dtype=np.uint8))
	    edges_dilated = cv2.dilate(imgReduc, np.ones((dilation_parameter,dilation_parameter), dtype=np.uint8))
	    ...
	    lines = cv2.HoughLinesP(edges_for_lines,1,np.pi/180,100,np.array([]),minLineLength,maxLineGap)
	    ...
	    img_and = cv2.bitwise_and(blackReduc, imgReduc)

	    img_and_dilated = cv2.dilate(img_and, np.ones((dilation_parameter,dilation_parameter), dtype=np.uint8))
	    ...
	    alpha = 0.45 #tolerance parameter for lines evaluation
	...
	global_true_positive = global_true_positive/size*100
	global_false_positive = global_false_positive/size*100
	global_true_negative = global_true_negative/size*100
	global_false_negative = global_false_negative/size*100
	global_lines_detected = global_lines_detected/size*100

	return global_true_positive, global_false_positive, global_true_negative, global_false_negative, global_lines_detected 


def roc_curve_sudoku():
	...
	for dilation_parameter in range (1,6):
	    true_pos, false_pos,_,_,_ = sudoku_lines_eval(dilation_parameter)
