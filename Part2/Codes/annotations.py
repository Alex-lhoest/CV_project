...
if __name__ == '__main__':
    ...
    for project in projects:
        with Cytomine(host=host, public_key=public_key, private_key=private_key,
            ...
            if params.opencv:
                image_instances = ImageInstanceCollection().fetch_with_filter("project", project)
            ...
            for annotation in annotations:
                # print("ID: {} | WKT: {}".format(annotation.id, annotation.location))
                ...
                if params.download_path:
                    path = os.path.join(params.download_path, "{project}", "{annotation.image}.txt")

                    with open(path, 'a') as outfile:
                        coord = re.sub("[^0-9. ]", "", str(geometry))
